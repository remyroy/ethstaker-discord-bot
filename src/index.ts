  ...
  return new Promise<void>(async (resolve, reject) => {
    db.serialize(() => {
      ...
      commandsConfig.forEach((config, key, map) => {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS ${tableName} (userId TEXT PRIMARY KEY UNIQUE NOT NULL, lastRequested INTEGER NOT NULL);`, (error: Error | null) => {
          if (error !== null) {
            reject(error);
          }
        });
...
const getLastRequested = function(userId: string, tableName: string) {
  return new Promise<number | null>(async (resolve, reject) => {
    db.get(`SELECT lastRequested from ${tableName} WHERE userId = ?;`, userId, (error: Error | null, rows: object | undefined ) => {
      if (error !== null) {
        reject(error);
      }
      if (rows === undefined) {
...
const storeLastRequested = function(userId: string, tableName: string) {
  return new Promise<void>(async (resolve, reject) => {
    db.serialize(() => {
      let doInsert = true;
      db.get(`SELECT lastRequested from ${tableName} WHERE userId = ?;`, userId, (error: Error | null, rows: object | undefined ) => {
        ...
        if (doInsert) {
          db.run(`INSERT INTO ${tableName}(userId, lastRequested) VALUES(?, ?);`, userId, lastRequested, (error: Error | null) => {
            if (error !== null) {
              reject(error);
            }
            resolve();
          ...
          db.run(`UPDATE ${tableName} SET lastRequested = ? WHERE userId = ?;`, lastRequested, userId, (error: Error | null) => {
            if (error !== null) {
              reject(error);
            }
            resolve();
...
client.on('interactionCreate', async interaction => {
  ...
  const userMention = Formatters.userMention(userId);
    ...
    const lastRequested = await getLastRequested(userId, tableName);
    let newRequestPart = '';
    ...
    try {
      ...
      if (targetBalance.gte(requestAmount)) {
        await storeLastRequested(userId, tableName);
